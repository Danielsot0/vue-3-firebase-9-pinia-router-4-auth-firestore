    ...
    <a-row>
        <a-col :xs="{ span: 24 }" :sm="{ span: 12, offset: 6 }">
            <a-form name="basicPerfil" autocomplete="off" layout="vertical" :model="userStore.userData" @finish="onFinish">
                ...
                <a-form-item name="displayName" label="Ingrese un Nick" :rules="[
                    {
                        required: true,
                        whitespace: true,
                ...
                <a-upload v-model:file-list="fileList" list-type="picture" :before-upload="beforeUpload"
                    @change="handleChange" class="mb-2">
...
const handleChange = (info) => {
    ...
    if (info.file.status !== 'uploading') {
        ...
        if (!isJpgOrPng) {
            message.error('Solo puedes subir archivos de tipo jpg o png');
...
const onFinish = async () => {
    ...
    if (fileList.value[0]) {
        ...
        if (error) {
            return message.error('Problemas al subir la imagen, intentalo de nuevo');
        }
        message.success('Se actualizo la imagen correctamte');
    }

    if (!error) {
        message.success('Se actualizó tu información correctamente')
    }
    else { message.error('Ocurrió un error') }
}
</script>
